...
corpusPath = 'Dataset\Retrieval.txt'
...
corpus, DID, distinct, doc_length = parseCorpus(corpusPath)
print("parse corpus file finished")
...
print("calculation step for corpus file finished")

# pd.DataFrame(TF_IDF_array).to_csv("data/TF_IDF_array")
# TF_IDF_array = np.array(pd.read_csv("data/TF_IDF_array"))
# print("read TF_IDF_array from saved file")

# extract data from Query files
QID = parseXML(queryPath, 'QID')
title = parseXML(queryPath, 'title')
description = parseXML(queryPath, 'description')
titleAndDescription = combineTwoArrays(title, description)
print("parse query file finished")

# extract data from judgment data file
judge = parseJudgment(judgmentPath)
print("parse judgment file finished")

# evaluate queries with their title, description, narrative using p@5 p@10 p@20
print("start evaluation")
precision = [5, 10, 20]
evalquery(title, precision)
evalquery(description, precision)
evalquery(titleAndDescription, precision)
